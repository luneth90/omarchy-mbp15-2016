...
- **彻底解决发热与显卡自由切换**：
  ...
  - **核显低温长续航模式 (`gpu-igpu`)**：通过 EFI 原生指令让内屏改由 Intel HD 530 驱动，AMD 独显进入深度节电待机档（214MHz / 0.75V），整机待机总功耗骤降至约 5.2W，掌托降至 34°C 温凉舒适状态；
...
> [!CAUTION]
> **切勿在系统运行中执行 `echo OFF > /sys/kernel/debug/vgaswitcheroo/switch`！**
> 在 2016 款配备 Touch Bar 的 15 寸 MacBook Pro（`MacBookPro13,3` / GMUX 4.0.29）上，内屏 eDP 硬件总线时钟与两侧显卡物理深度耦合。如果在运行中强行断开独显供电轨，会导致 Intel 核显链路同步丢失（Link Training 失败），屏幕瞬间黑屏死机。
> 本套件通过 **EFI 模式切换 + 驱动级深度休眠锁频（214MHz / 0.75V）** 实现安全退烧（整机待机总功耗仅约 5.2W，掌托 34°C），既稳定又安全。
