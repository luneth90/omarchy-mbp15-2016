...
`MacBookPro13,3` 在通过非 macOS 启动时，固件默认只会暴露 AMD 独显。若想切换至 Intel 核显（降低发热并延长续航），需要引导器配合激活核显。

> **注意**：外接 USB-C 视频输出硬件连接在 AMD 独显上。切换至 iGPU 后，外接显示器通常不可用，请务必拔掉外接屏幕后再进行操作。

### 步骤 1：引导器配置 `apple_set_os`
在引导器中配置每次启动 Linux 前执行 `apple_set_os.efi`（例如在 rEFInd 的 `refind.conf` 中配置 `spoof_osx_version 10.12`）。保持当前 AMD 独显重启。

### 步骤 2：确认核显已暴露并由内核接管
重启后，在终端中确认 Intel 核显可见且驱动已正常加载：
```bash
lspci -nnk -s 00:02.0
```
输出应显示 Intel 显卡，且包含 `Kernel driver in use: i915`。若未显示则切勿继续操作。
...
sudo reboot
```
> **安全回退机制**：为防止内屏黑屏无法进入系统，脚本在首次切换 iGPU 时会自动装载回退保护——如果本次启动未人工确认，下一次重启将自动恢复为 AMD 独显模式。

### 步骤 4：验证并确认保留 iGPU
重启进入桌面后，确认显示、键盘、触控板等一切正常，执行：
```bash
sudo ./omarchy-mbp15-2016.sh verify-gpu
sudo ./omarchy-mbp15-2016.sh gpu-confirm-igpu --yes
```
执行确认后，下次启动将永久保留 Intel 核显。
